...
subjCa="/C=${OPENSSL_CERTIFICATE_SUBJECT_COUNTRY}/ST=${OPENSSL_CERTIFICATE_SUBJECT_STATE}/L=${OPENSSL_CERTIFICATE_SUBJECT_LOCATION}/O=${OPENSSL_CERTIFICATE_SUBJECT_ORGANIZATION}/OU=${OPENSSL_CERTIFICATE_SUBJECT_ORGANIZATION_UNIT}/CN=${OPENSSL_CERTIFICATE_SUBJECT_ORGANIZATION}"

openssl genrsa -out "${OPENSSL_CERTIFICATES}/${SITE_DOMAIN}.rootCA.key" ${password_out} "${OPENSSL_KEY_SIZE}"
openssl req -x509 -new -nodes -key "${OPENSSL_CERTIFICATES}/${SITE_DOMAIN}.rootCA.key" ${password_in} "-${OPENSSL_CERTIFICATE_HASH}" -days "${OPENSSL_CERTIFICATE_EXPIRATION}" -out "${OPENSSL_CERTIFICATES}/${SITE_DOMAIN}.rootCA.crt" -subj "$subjCa"
